...
class ShipmentController extends Controller
{
    ...
    private function storeShipment($request , $token = null)
    {
        ...
        if( auth()->user()->role == 4 ){ // IF IN AUTH USER == CLIENT
    ...
    public function getShipmentsAPI(Request $request)
    {
        try {
            ...
            if($user){
                $userClient = Client::where('user_id',$user->id)->first();
                $shipments = Shipment::where('client_id',$userClient->id)->with(['pay','from_address'])->get();
            ...
            foreach ($data as $row) {
                for ($i=0; $i < count($row); $i++) {
                    ...
                    if($request->columns[$i] == 'payment_method_id'){
                        ...
                        $payment = json_decode($paymentSettings[$row[$i]], true);
                        if(!$payment){
