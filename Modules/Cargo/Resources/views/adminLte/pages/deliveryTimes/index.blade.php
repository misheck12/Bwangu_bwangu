...
@section('pageTitle')
    {{ __('cargo::view.delivery_time_list') }}
@endsection
        ...
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-toolbar">
                ...
                <div class="d-flex flex-wrap align-items-center" id="{{ $table_id }}_custom_filter">
                    ...
                    @if(auth()->user()->can('export-table-delivery-time') || $user_role == $admin )
                    ...
                    @if(auth()->user()->can('create-delivery-time') || $user_role == $admin )
                        <a href="{{ fr_route('deliveryTime.create') }}" class="btn btn-primary m-1">{{ __('cargo::view.add_delivery_time') }}</a>
                    @endif
                    <!--end::Add user-->
                </div>
                <!--end::Toolbar-->
                ...
                @include('adminLte.components.modules.datatable.columns.checkbox-actions', [
                    'table_id' => $table_id,
                    'permission' => 'delete-delivery-time',
                    'url' => fr_route('deliveryTimes.multi-destroy'),
                    'callback' => 'reload-table',
                    'model_name' => __('cargo::view.selected_delivery_times')
                ])
                <!--end::Group actions-->
