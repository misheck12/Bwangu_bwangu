    ...
    <section id="integration" class="section py-0 mt-5 mb-5 p-3"
        ...
        <div class="container-fluid">
            <div class="row align-items-center flex-row-reverse">
                <div class="col-md-6">
                    <div class="intro">
                        ...
                        <div class="actions d-flex flex-column flex-md-row align-items-start">
                                <img  width="55" height="55"   src="{{ theme_setting_image('homepage.integration.section_banner1') != ''? theme_setting_image('homepage.integration.section_banner1'): get_general_setting('website_logo', asset('themes/qwintry/assets/images/icon-code.svg')) }}" alt="icon code" />
                            <div class="d-flex flex-column">
                                @php
                                    $button_type = $data['link2']['type'];
                                @endphp

                                <div class="d-flex flex-column align-items-center flex-md-row">
                                    <form  class="mb-3" >
                                        <a href="{{ $data['link1'][$button_type] ?? '' }}" class="btn  btn-lg fa-10x" style="{{ $buttonColor }} {{ $buttonBgColor }}"> {{ $data['section_button'][app()->getLocale()] ?? '' }}
                                ...
                                <form action="{{ $data['link2'][$button_type] ?? '' }}">
                                    <button type="submit" class="btn " style="{{ $buttonColor1 }} {{ $buttonBgColor1 }}"> {{ $data['section_button1'][app()->getLocale()] ?? '' }} </button>
                                </form>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-6">
                    <div class="wrapper" >
                        <img  style=" height: 516px; width: 395px; max-width: 100%; "  src="{{ theme_setting_image('homepage.integration.section_banner2') != ''? theme_setting_image('homepage.integration.section_banner2'): get_general_setting('website_logo', asset('themes/qwintry/assets/images/mobile.png')) }}" alt="mobile" />
                    </div>
                </div>
            </div>
        </div>
