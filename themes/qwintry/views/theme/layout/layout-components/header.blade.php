...
<header id="main-header" class="active {{ array_key_exists('header_style', $header_setting) && $header_setting['header_style'] == 'dark_style' ? "dark_style" :  'light_style'  }}">
    ...
    <nav id="navigation" class="navbar dropdown-light" aria-label="primary" >
        <div class="container">
            <div class="d-flex justify-content-center align-items-center">
                <div  class="logo site--logo">
                    <a href="{{ fr_route('home') }}" rel="home" title="{{ config('app.name') }}">
                        <img style=" max-height: 60px;  max-width: 250px; "  src="{{ theme_setting_image('homepage.banner.section_banner') != '' ? theme_setting_image('header.header.header_logo') : (get_general_setting('website_logo', asset('themes/qwintry/assets/images/logo.svg'))) }}" alt="banner" class="banner" />
                    </a>
                </div>
            ...
            <div class="mobile-items" style="display: flex ; align-items: center;" style="">
                ...
                <!-- dropdowns -->
                    ...
                    <ul class="nav-components bd-components" >
                        @if (array_key_exists('display_sign_in', $header_setting) && $header_setting['display_sign_in'])
                            ...
                            @if (array_key_exists('display_sign_in', $header_setting) && $header_setting['display_sign_in'])
                                <a href="{{ fr_route('admin.dashboard') }}" class="btn d-none d-lg-inline px-3" style="{{ array_key_exists('header_text_color', $header_setting) && $header_setting['header_text_color'] ? "color: {$header_setting['header_text_color']};" :  ''  }}">@lang('theme_easyship::view.signin')</a>
                            @endif
                            <button id="nav-open" class="btn btn-sm px-3 bars d-lg-none stop-propagation" aria-label="navbar toggle" style="{{ array_key_exists('header_text_color', $header_setting) && $header_setting['header_text_color'] ? "color: {$header_setting['header_text_color']};" :  ''  }}">
                                <i data-feather="menu"></i>
                            </button>
                                ...
                                <div class="bdaia-ns-content">
                                    <div class="bdaia-ns-inner">
                                        <form class="form" action="{{route('shipments.tracking')}}" method="GET">
                                            <input type="text" class="bbd-search-field search-live" id="code" name="code" value="Enter your tracking code"
                                                onfocus="if (this.value == 'Enter your tracking code') {this.value = '';}"
                                                onblur="if (this.value == '') {this.value = 'Enter your tracking code';}" autocomplete="off" />
...
<nav aria-label="primary mobile" class="drawer d-lg-none">
    <header class="header d-lg-none d-flex justify-content-between align-items-center">
      <img src="{{ theme_setting_image('header.header.header_logo') != '' ? theme_setting_image('header.header.header_logo') : asset('assets/lte/cargo-logo-small-h38.svg') }}" alt="{{ config('app.name') }}" />
    ...
    <div class="body">
      <ul class="nav-list list-unstyled mb-0">
        @if (array_key_exists('display_home_icon', $header_setting) && $header_setting['display_home_icon'])
            <li class="nav-item">
                <a class="nav-link" href="{{ fr_route('home') }}" style="{{ array_key_exists('header_text_color', $header_setting) && $header_setting['header_text_color'] ? "color: {$header_setting['header_text_color']};" :  ''  }}" data-toggle="dropdown">@lang('theme_easyship::view.home') </a>
            </li>
        @endif
        {!! get_menu_header() !!}
      </ul>
